...
1. **`build_maps.py`** reads `jp_city_v2.csv` (the hand-curated bilingual source list and the source of truth: `prefecture_jp, city_jp, prefecture, city, population`) and in one pass:
   ...
   - The committed `map/` tiles were generated with `--provider stadia` (label-free `stamen_toner_background`), so they are black-and-white.
...
- **`stadia`** — **free for non-commercial use** and the recommended free backend. Env `STADIA_API_KEY`. Geocodes then fetches static (**2 requests/tile**); static center is `lat,lon`. Default style `stamen_toner_background` is label-free with no setup. The `stamen_*_background` slugs render even though Stadia's theme docs omit them. Verified working end-to-end June 2026.
...
- **stadia** (current committed maps): label-hiding is a property of the *style*. Default `stamen_toner_background` is label-free; `stamen_*_background` slugs work even though Stadia's theme docs omit them.
